...
! The bibliography file should only have bibliograpgy and bibitem tags
    ...
    integer, parameter :: unit3=23  ! parameter file read once and then closed
...
! allocate arrays for storing data, related to number of elements
       ...
       if(debug) write(*,9)'Dimensioning:',&
            maxel,maxsp,maxph,maxpar,maxtp,maxbib
9      format(a,6i7)
...
18     format('Read xtdbtag: ',3i5,' "',a,'" "',a,'"')
...
! reading bibliography AppendXTB if there is no AppendXTDB for TPfuns
                ...
                do ntp=1,nselbib
                   if(selbib(ntp)%status.lt.0) missingbib=missingbib+1
                enddo
!                write(*,*)'We are here',rewinds
!                if(missingtp.gt.0 .and. missingtp-prevmisstp.gt.0) then
                if(missingtp.gt.0 .and. rewinds.lt.3) then
                   allappy=3
                   fline=0
                   rewinds=rewinds+1
                   write(*,322)trim(masterfile),missingtp-prevmisstp,rewinds
322                format('Nested TPfuns, rewinding: ',a,2i3)
...
!             write(*,*)'In xtdbread TPfun: ',trim(tpfun)
             call xtdbentertpfun(tpfun,add)
             if(add) then
                call OCentertpfun(tpfun)
             endif
...
!             write(*,38)trim(attname),trim(values)
          enddo
       elseif(tagname(1:lc).eq.'Liq2State ') then
!------------------------------------------
!       case(24) ! Liq2state
...
! make sure we prepare an endoftag
    enddo bibitems
1000 continue
    if(debug) write(*,*)'Found ',found,' missing bibliographic references'
    close(unit)
    return
! errors model
1100 continue
...
312 format(/'Back after reading XTDB file: ',a,', error: ',i5)
  if(xtdberr.ne.0) then
     write(*,313)xtdberr,trim(filename)
313  format('Error ',i5,' when reading XTDB file: ',a)
  endif
