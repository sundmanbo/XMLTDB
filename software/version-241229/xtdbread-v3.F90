...
!   
! an expression may be concatinated from TPfun/Parameter and Trange tags
...
! this array will have the seosp indices in alpahetical order
...
! in this array only selected constituents are entered
    ...
    type(ocparam), dimension(:), allocatable :: selpar
...
! We need also seltp(*)%tatus if missing or found
! we should replace all alltpfun with seltpfun
...
!    integer, dimension(:), allocatable :: statusbib
...
! for dimensioning these arrays and amount used
    integer maxel, maxsp, maxph, maxpar, maxtp, maxbib
!==================================================================
  contains
!==================================================================
...
!    matt=0 a complte tag found with tagname and attributes
...
! The bibliography file should only have bibliograpgy and bibitem tags
    ...
    integer, parameter :: unit2=22  ! model file open a short time initially
    integer, parameter :: unit3=23  ! parameter file read once and theb closed
    integer, parameter :: unit4=24  ! tpfun file may be rewinded several times
    integer, parameter :: unit5=25  ! bibliograpphy read at the end with
                   ! with references for the parameters
!
! for nested tags level is current level,  matt=0 if tagend missing (nesting)
    character pretag*24,values*256,attname*18
    integer tagno,matt,ll
...
! Bibilographic references
    ...
    character ch1*1,spel*24,spname*24,stoichiometry*60,mqmqa*40,uniquac*12
    character stoisp*40
! for disorderedpart
...
! for various purposes
    ...
    integer missbib,unknowntp,jel,skip
!
...
! allocate arrays for storing data, related to number of elements
       listphases=.true.
       maxel=nel+2; maxsp=10*nel*(nel-1); maxph=5*nel*(nel+1); 
       maxpar=20*nel*(nel+1)*(nel+2)
       maxtp=30*nel*(nel+1); maxbib=5*nel*(nel+1)*(nel+2)
       allocate(selel(-1:maxel))
       write(*,9)'Dimensioning:',maxel,maxsp,maxph,maxpar,maxtp,maxbib
9      format(a,6i7)
! ternary: 5 7 60 120 60
!Include Va and /- as element -1 and 0
       selel(-1)%elname='/-'; selel(-1)%data=' '
       selel(0)%elname='VA'; selel(0)%data=' '
       allocate(selsp(maxsp))
...
! we may have end of file reading an AppendXTDB files here ...
          if(nel.eq.0) then
             nel=jel
          else
             write(*,*)'Back with xtdberr: ',xtdberr,parappy,tpfunappy
             if(xtdberr.eq.4700) then
                write(*,*)'EOF reset'
                xtdberr=0
             else
!          endif
! this closes the file after extracting elements
...
!                   write(*,*)'Ignoring: ',trim(values),len_trim(values)
                ...
                call OCenterel(values,attributes)
                cycle readall
             enddo
          endif
       endif
...
46           format('Line ',i7,' Tags: ',5(i3,2x,a))
          endif
...
! The ModelXTDB is read when found, the other will be opened for read here
             write(*,*)'End of file for: ',trim(currentfile)
             ...
             write(*,*)'Number of bibitem found and missing:',nselbib,missingbib
...
47           format('Any AppendXTDB files to read?',4i3)
             write(*,*)'Press return to continue',tpfunappy
             read(*,'(a)')ch1
! if there is an AppendXTDB for Parameters read that now!
! The Parameter tag is done first
             if(parappy.eq.1) then
                write(*,*)'Appending Parameter file: ',trim(parappx)
!----------------------------------------------------------
...
! parappy set to 0 as it it read only once
                ...
                write(*,*)' *** Opened ',trim(parappx),' line number ',fline
                currentfile=parappx
...
! Either all TPfuns found or the missing are not on tpfunappx
                   write(*,*)'No more TPfuns, closing: ',trim(tpfunappx)
                   tpfunappy=0
                   close(unit)
                endif
...
! the file is opended and read separatelym only bibitems allowed
                   ...
                   write(*,*)'Back from reading bibliography'
                   missingbib=0
                   do ntp=1,nselbib
                      if(selbib(ntp)%status.lt.0) missingbib=missingbib+1
                   enddo
...
! a final try rewinding and  read from primary file
                   unit=unit1
                   currentfile=masterfile
                   rewind(unit)
                   cycle readall
...
! There may also be a <Bibliography> tag in the main file? 
                write(*,*)'Appending bibliography 2: ',trim(biblioappx)
                biblioappy=0
                call xtdbbiblio(biblioappx)
                write(*,*)'Back from reading bibliography'
                prevmisstp=missingtp
...
! to avoid rewind masterfile forever
                   prevmisstp=0
                   fline=0
                   write(*,*)'Found missing data, rewinding: ',masterfile,fline
...
321                format('Finished 1 reading XTDB file: ',a)
                   unit=unit1
                   goto 990
                endif
             else
! reading bibliography AppendXTB when there is no AppendXTDB for TPfuns
                write(*,*)'Applied all AppendXTDB (or none)'
                missingtp=0
                do ntp=1,nseltp
                   if(seltpfun(ntp)%status.lt.0) missingtp=missingtp+1
                   enddo
                ...
                do ntp=1,nselbib
                   if(selbib(ntp)%status.lt.0) missingbib=missingbib+1
                enddo
                write(*,*)'We are here!',missingtp,missingtp-prevmisstp
                if(missingtp.gt.0 .and. missingtp-prevmisstp.gt.0) then
                   allappy=3
                   fline=0
                   write(*,*)'Found data 3, rewinding: ',trim(masterfile),&
                        missingtp-prevmisstp,unit1
...
!======================== below is treating end of nested tags from any file
...
!             write(*,38)trim(attname),trim(values)
          ...
          enddo
          call OCenterel(trim(tpfun),trim(attributes))
!------------------------------------------
       case(6) ! Species
          ip=1; values=' '
...
!             if(tpfun(1:7).eq.'GFECEM ') write(*,*)'GFECEM ',trim(wholexpr)
          endif
!------------------------------------------
       case(8) ! Trange  
          if(matt.gt.0) then
...
!          write(*,*)'Found DisorderedPart line ',fline
          ...
          do while(getatt(attributes,ip,attname,values))
!             write(*,38)trim(attname),trim(values)
             if(attname(1:10).eq.'Disordered') disph=trim(values)
...
!             write(*,*)'Ordered sublattices: ',kk,phnsub
             if(kk.gt.phnsub .or. kk.lt.2) then
                write(*,*)'DisorderedPart has too large or small:',kk,phnsub
                xtdberr=4444; goto 1000
             endif
          endif
!          write(*,*)'Save sum',allocated(disph),allocated(sub)
          phrec%dispar=sum
! the code went crazy here when using a non-allocated character
          if(allocated(disph)) then
             phrec%dispar=phrec%dispar//' '//disph
          endif
!          write(*,*)'Save sum: ',phrec%dispar,allocated(disph),allocated(sub)
...
!------------------------------------------
       case(17) ! Parameter
! just for fun, when first parameter entered list all entered phases
          listphases=.false.
          if(listphases) then
             listphases=.false.
             write(*,68)nselph
...
! assume Id is always before text
          ip=1; values=' '
!          write(*,*)'compare',nselbin
!          do kk=1,nselbib
!             write(*,*)'Bibitem: ',selbib(kk)%bibitem,selbib(kk)%status,kk
!          enddo
...
!             write(*,*)'Bibitem: "',bibref(1:lk),'" line:',fline
             do kk=1,nselbib
!                write(*,374)kk,bibref(1:lk),selbib(kk)%bibitem(1:lk),&
!                     selbib(kk)%status
374             format('Bibref: ',i3,' "',a,'" and "',a,'" Status ',i3)
                if(bibref(1:lk).eq.selbib(kk)%bibitem(1:lk)) then
!                   write(*,*)' Heurecka!!',fline
...
!                   elseif(allappy.eq.0) then
! ignore this when rewinding masterfaile same bibitems will appear
!                      write(*,*)'*** Warning bibref "',bibref(1:lk),&
!                           '" is duplicate'
                   endif
                endif
             enddo
...
990 if(.not. ignorEOT) then
       write(*,*)'End of file',unit
       write(*,993)nselel,nselsp,nselph,nselpar,nseltp,nselbib
993 format('nselel ',i4,' nselsp ',i4,' nselph ',i4,' nselpar ',i4,&
         ' nseltp ',i5,' nselbib ',i4)
       missingtp=0
       do ntp=1,nseltp
          if(seltpfun(ntp)%status.lt.0) missingtp=missingtp+1
       enddo
...
102    format(/'xtdbtag mode ',i3,' line ',i6,' "',a,'"')
...
88                    format(' *** Rubbish after > line ',i7,&
                           '. Quit reading.'/a/)
                      xtdberr=4766
                      exit readtag
                   endif
...
!          write(*,*)'Modelinfo Einstein  23'
          ...
          do while(getatt(attributes,ip,attname,values))
             write(*,38)trim(attname),trim(values)
          enddo
       elseif(tagname(1:lc).eq.'Liquid ') then
!------------------------------------------
!       case(24) ! Liquid2state
!          write(*,*)'Modelinfo Liquid2State  24'
          ip=1; values=' '
...
!                   write(*,555)values(1:lbib),trim(selbib(nbib)%bibitem)
555                format('Compare "',a,'" "',a,'"')
...
60                       format('Formatting error file: "',a,'" line ',i7)
                      endif
                   endif
                enddo find
             endif
...
! make sure we prepare an endoftag
    enddo bibitems
1000 continue
    write(*,*)'Found ',found,' missing bibliographic references'
    close(unit)
    return
! errors model
1100 continue
...
! Strange error here when everyting was accepted as TPfuns ....
    ...
    call capson(line)
    kp=0
!    write(*,*)'Looking for tpfun in: "',trim(line),'"',ip,kp
    symbol=' '
...
77           format('istpfun: Too long symbol: "',a,'" ',2i4,' on line '/a)
             stop
          endif
...
!    write(*,*)'Enter selected element: ',spel
    nselel=nselel+1
    selel(nselel)%elname=spel
    selel(nselel)%data=data
! Hm alphabetical order!!
!    write(*,10)nselel,spel,trim(data)
10  format('Selected element; ',i2,2x,a,' : ',a)
...
!          write(*,*)'The species ',trim(spname),' has unknown element ',el(ia)
!          write(*,*)'Ignoring species with ',trim(stoi)
          goto 1000
       endif
    enddo thissp
    nselsp=nselsp+1
...
!       selsp(nselsp)%stoicc(nel)=qq
    ...
    if(mqmqa(1:1).ne.' ') selsp(nselsp)%extra=mqmqa
...
!
    return
1100 continue
! We arrive here there is a sublattice without any entered constituents
...
! shift data in ia and ia+1, very clumsy
                selel(maxel)%elname=selel(ia)%elname
                selel(maxel)%data=selel(ia)%data
                selel(ia)%elname=selel(ia+1)%elname
                selel(ia)%data=selel(ia+1)%data
                selel(ia+1)%elname=selel(maxel)%elname
                selel(ia+1)%data=selel(maxel)%data
             endif
          enddo
...
!       write(*,*)'Order species alphabetically in selspord'
       ...
       enddo
!       do ia=1,nselsp
!          write(*,90)ia,trim(ss(ia)),selspord(ia)
90        format('Specie ',i3,': "',a,'" ',i5)
!       enddo
!
!       write(*,*)'Ordering species alphabetcally',nselsp
       kk=0
       ord2: do while(.true.)
...
!-----------------------
! Enter this parameter, the parameter may need new TPfuns needed, use wholexpr
...
! The phase name may be abbreviated between "_" characters
    character*(*) parname
    character*24 phasename,constituent,mpid*8
...
!
    ...
    lk=index(parname,'(')
    if(lk.le.0) then
       write(*,*)'Missing ( in parameter: ',trim(parname)
...
! we have an unknown MPID
    write(*,*)'Unknown Model Parameter IDentification (MPID): "',mpid,'"'
...
! there must be a , after the phase name lk+1 is the first letter
    ...
    if(lm.le.0) then
       write(*,*)'Missing "," after phase name in parameter: ',trim(parname)
       write(*,*)'Parameter: "',trim(parname),'"'
...
!  subroutine xtdbselectspecies
! generate set of selected species/constituents from selected elements
!    write(*,*)'xtdbselectspecies not yet written'
! all species that can exit with the current set of selected elements
!    
!    return
!  end subroutine xtdbselectspecies
...
program xmltest
  use xtdblib
  implicit none
  character filename*60,line*60,charge*10,spline*128,ch1*1
  character*2, dimension(90) :: elnames
  integer ip,ie,nel,nn,kk,jj,lk
  double precision coefs(90)
!  
  sptest: do while(.false.)
! skip this
     xtdberr=0
     write(*,*)'Testing stoichiometries:'
     read(*,7)filename
7    format(a)
     if(filename(1:1).eq.' ') exit sptest
     call extractstoi(filename,nel,elnames,coefs)
     if(xtdberr.ne.0) write(*,*)'xtdberr: ',xtdberr
     do ip=1,nel
        write(*,19)elnames(ip),coefs(ip)
19      format(a,2x,F10.4)
     enddo
  enddo sptest
!  
  write(*,*)'XML file name:'
  read(*,*)filename
! just extract element names
  ie=0
! call with ie=0 extract all elements in the database in elnames
  call xtdbread(filename,ie,elnames)
!
  write(*,100)(elnames(ip),ip=1,ie)
100 format(/'Elements in database: ',50(a,1x))
  write(*,*)'Select elements:'
  read(*,'(a)')line
  write(*,*)'Selected: ',trim(line)
  ip=0
  ie=0
  do while(.not.xeolch(line,ip))
! xeolch return true when no more elements
!     write(*,*)'Position: ',ip,': ',line(ip:ip+1)
     ie=ie+1
     if(line(ip+1:ip+1).ne.' ') then
        elnames(ie)=line(ip:ip+1)
        ip=ip+2
     else
        elnames(ie)=line(ip:ip)
        ip=ip+1
     endif
  enddo
  write(*,100)'Selected Elements: ',(elnames(ip),ip=1,ie)
  if(ie.gt.0) then
     call xtdbread(filename,ie,elnames)

! fline not always relevant as we may read from several files
!
     write(*,312)trim(filename),xtdberr
312  format(/'Back after reading XTB file: ',a,', error: ',i5)
...
!21         format('Species: ',i3,2x,a,5x,10(2x,a,1x,f12.6))
        enddo
     endif
!
...
298  format(/'Phases:')
     do ip=1,nselph
        write(*,69)trim(selph(ip)%phasename),selph(ip)%confent,&
             ...
             trim(selph(ip)%const)
69      format('Phase: ',a,2x,a,'  Subl: ',i2,'  Mult: ',a/&
             '   Constituents: "',a,'"')
