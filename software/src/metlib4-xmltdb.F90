...
! Replacing OCPARAM
  double precision, parameter :: zero=0.0D0, one=1.0D0, two=2.0D0, ten=1.0D1, &
...
subroutine skip_to_space(line,ip)
! skip to next space character (end of text item)
    integer ip
    character line*(*)
! can be faster using index( on a substring
    do while(line(ip:ip).ne.' ')
       if(ip.gt.len(line)) goto 2000
       ip=ip+1
    enddo
1000 continue
    return
2000 continue
    xmlerr=5000
    goto 1000
  end subroutine skip_to_space

!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/

subroutine skip_to_char(ch1,line,ip)
! skip to next space character (end of text item)
    integer ip
    character line*(*),ch1*1
! can be faster using index( on a substring
    do while(line(ip:ip).ne.ch1)
       if(ip.gt.len(line)) goto 2000
       ip=ip+1
    enddo
1000 continue
...
2000 continue
    xmlerr=5000
    goto 1000
  end subroutine skip_to_char

!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/!\!/

subroutine skip_to_first(charray,kp,line,ip)
! skip to next space character (end of text item)
    integer ip,kp,lenl
    character line*(*),charray*(*)
    lenl=len(line)
    lent=len(charray)
    search: do ip=ip,lenl
       do kp=1,lent
!          write(*,*)'skip_to_first: ',ip,line(ip:ip),charray(kp)
          if(line(ip:ip).eq.charray(kp:kp)) goto 1000
       enddo
    enddo search
    xmlerr=5000
1000 continue
    return
  end subroutine skip_to_first
