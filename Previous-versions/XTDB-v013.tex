...
\begin{center}
  ...
  {\Large \bf Proposal 0.1.3 of an XML format to replace TDB files}
...
add text with {\color{green} with your own color}
...
the Appendix~\ref{sec:examples} there are several examples of tags and
attributes, in Appendix~\ref{sec:modelapp} an extensive list of model
tags and attributes.  In Appendix~\ref{sec:partags} some explanations
how particular thermodynamic model features can be integrated in the
XTDB format including the use of wildcards and the EBEF.  In
Appendix~\ref{sec:complete} the complete XTDB file for the binary Al-C
and Al-Li are listed and finally in Appendix~\ref{sec:changes} an
...
the recommended order is {\bf Defaults, DatabaseInfo} and any {\bf
  AppendXTDB} tags followed by {\bf Element, Species} and {\bf Phases}
(with many nested tags).

The main part of the file are the {\bf TPfuns} and {\bf Parameter}s
tags, which can be ordered by phases or for several phase together in
a {\bf BinarySystem} or {\bf TernarySystem} tag.  The {\bf
  TernaryXpol} tags should be together with the phase or the binary
parameters.

The {\bf Bibliography} and {\bf Model} tags are normally at the end or
in a separate{\bf AppendXTBD} file.  The {\bf AppendXTDB} tag, which
specifices an additional file of an XTDB database allows the database
to be split on several files.  The {\bf Default, DatabaseInfo} and
{\bf AppendXTDB} tags and all {\bf Element, Species} and {\bf Phase}
...
capital letter.  The exceptions are the {\bf Disordered\_2State} and
{\bf Disordered\_3State} tags.
...
!              & List &  Species {\em Id} in the sublattice,
  ...
  Disordered\_2Part & & Optional tag inside the {\bf Phase} tag of an 
              ...
              eq.~\ref{eq:nosub} below.  See also section~\ref{sec:dispart}.
...
!  & Sum & Number of sublattices, starting from the first sublattice in the
   ...
   Disordered\_3Part & & Optional tag inside the {\bf Phase} tag of an
              ...
              Similar to Disordered\_2Part but use eq.~\ref{eq:sub} below.\\
...
See also the Appendix~\ref{sec:modelapp}.
...
constituents of indentical sides.
...
described by the GEIN function in the {\em Expr} attribute in the {\bf
  Parameter} or {\bf TPfun} tag, {\color{magenta} the GEIN function
  defined in eq.~\ref{eq:gein} in section~\ref{sec:expr}.}  For
multiple $\theta$ see the Al-C example in Appendix~\ref{sec:alc}.
...
{\color{magenta} This tag can be used also in an ebcrypted database
  for the systems that are assessed in the database for the concerned
  user.  It is important to show the users that databases are not not
  created by computers or appear from thin air.}
...
can also be omitted.  {\color{magenta} Stoichiometric fractions can be
  specified as AlO3/2 or by real numbers such as AlO1.5 but avoid
  numbers such as 0.333333, use 1/3.}

{\bf No grouping using parenthesis are allowed when specifying the
  stoichiometry, for example use AL2S3O12, not Al2(S1O4)3.}  It may
not be very elegant but computers are less able to understand nice
formatting.
...
$\theta$ in Kelvin.  In the corresponding LNTH parameter one will also
use the value of $\theta$ although the software will vary the
logarithm of $\theta$ with the composition as this is more reasonable
physically than varying $\theta$ itself.
...
\begin{enumerate}
\item The ternary extrapolation method nas been revised.  An attribute
  {\em TernaryXpol} added to the {\bf Defaults}.
...
\item {\color{red} A phase constituent must have a defined {\em Id} in
  a {\bf Species} tag in order to be used for listing or plotting the
  phase constitution.}
...
{\color{magenta} As a special case one can use

\begin{verbatim}
<TernaryXpol Phase=``Liquid'' Constituents=``*'' Xmode="KKK" />
\end{verbatim}

If all ternary extrapolations are Kohler.  More elaborate cases have
to be discussed, for example sorting the phase constituens in groups
and use Kohler if all 3 constituents are in the same group and Toop if
there is one from a different group.}
...
\begin{enumerate}
\item {\color{magenta} Some minor changes from 0.1.2 by Bosse using
  this color.}
...
\item The previous {\bf Toop} and {\bf Kohler} tags are integrated in
  a new {\bf TernaryXpol} tag following the suggestions in the paper
  by Pelton~\cite{01Pel}.
